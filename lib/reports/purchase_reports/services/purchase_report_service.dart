...
class PurchaseReportService {
  ...
  Future<List<PurchaseModel>> getAllVouchers(
      {int? supplierId, Map? date}) async {
    List datas = await purchaseRepository.getAllVouchers(
        supplierId: supplierId, date: date);
    return datas.map((data) => PurchaseModel.fromMap(data)).toList();
  }

  Future<List<PurchaseItemModel>> getPurchaseItems(
      {int? catId, Map? date}) async {
    List datas =
        await purchaseRepository.getSaleItems(catId: catId, date: date);
  ...
  Future<List<Map<String, dynamic>>> getMonthlyPurchase({
    ...
    return await purchaseRepository.getMonthlyPurchase(year: year);
  }

  Future<List<Map<String, dynamic>>> getYearlyPurchase() async {
    return await purchaseRepository.getYearlyPurchase();
  }
}
